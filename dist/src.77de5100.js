...
function render() {
  ...
  for (var _i = 0, strokes_1 = strokes; _i < strokes_1.length; _i++) {
    ...
    for (var _a = 0, stroke_1 = stroke; _a < stroke_1.length; _a++) {
      var point = stroke_1[_a];
  ...
  for (var _b = 0, targetvalues_1 = targetvalues; _b < targetvalues_1.length; _b++) {
    var targetStroke = targetvalues_1[_b];
    context.beginPath();
    curve.lineStart();

    for (var _c = 0, targetStroke_1 = targetStroke; _c < targetStroke_1.length; _c++) {
      var point = targetStroke_1[_c];
      curve.point(point[0], point[1]);
    }

    if (targetStroke.length === 1) curve.point(targetStroke[0][0], targetStroke[0][1]);
    curve.lineEnd();
    context.lineWidth = LINEWIDTH;
  ...
  for (var _d = 0, aiguess_1 = aiguess; _d < aiguess_1.length; _d++) {
    var aiStroke = aiguess_1[_d];
    context.beginPath();
    curve.lineStart();

    for (var _e = 0, aiStroke_1 = aiStroke; _e < aiStroke_1.length; _e++) {
      var point = aiStroke_1[_e];
      curve.point(point[0], point[1]);
    }
...
d3.select(context.canvas).call(d3.drag().container(context.canvas).subject(dragsubject).on("start drag", dragged).on("start.render drag.render", render)); // Create a new empty stroke at the start of a drag gesture.
...
function dragged() {
  var newX = d3.event.x;
  var stroke = d3.event.subject;

  if (stroke) {
    var maxX = Math.max.apply(Math, stroke.map(function (strokeSegment) {
      return strokeSegment[0];
    }));
